...
TERMINAL_CLOSE_STATES = frozenset(
    {"cancelled", "canceled", "filled", "rejected", "failed", "voided"}
)
...
def emergency_close_generation(
    ...
    *,
    ...
    """Terminal sell-to-close count for this instrument. Both fires see the same book."""
    oid = (option_id or "").strip()
    count = 0
    for row in option_orders or []:
        if oid not in order_option_ids(row):
        ...
        if not is_sell_to_close(row):
            continue
        if normalize_state(row.get("state") or row.get("status")) in TERMINAL_CLOSE_STATES:
            count += 1
    return count
...
def decide_emergency_close(
    *,
    ...
    covered = working_close_quantity(option_orders, option_id=oid)
