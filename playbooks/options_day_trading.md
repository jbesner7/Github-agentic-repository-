...
- Pattern math is locked in `rules.json` `agent_h.patterns` and `pipeline/patterns.py`: H&S order and 1.5% head prominence, 3-bar pivot separation, max duration 60 daily / 40 hour bars, triangle OLS plus 2 touches/side, deterministic overlapping-pattern rank
